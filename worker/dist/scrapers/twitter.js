...
exports.scrapeTwitter = scrapeTwitter;
exports.postTweet = postTweet;
exports.interact = interact;
exports.follow = follow;
exports.scrapeMentions = scrapeMentions;
...
const USER_DATA_DIR = path_1.default.resolve(process.cwd(), '../browser_profiles/twitter_automation_profile');
/**
...
async function getContext() {
    ...
    if (!fs_1.default.existsSync(USER_DATA_DIR)) {
        ...
        logger_1.logger.info(`Using existing browser profile: ${USER_DATA_DIR}`);
    }
    return await playwright_extra_1.chromium.launchPersistentContext(USER_DATA_DIR, {
...
async function scrapeTwitter(job) {
    const { targetUrl } = job.data;
    logger_1.logger.info(`Launching Twitter scraper for ${targetUrl || 'home timeline'}`);
    ...
    try {
        ...
        if (page.url().includes('login') || page.url().includes('logout')) {
            logger_1.logger.warn('Session invalid, redirecting to login. Manual intervention may be required if headless.');
        }
        ...
        logger_1.logger.error(`Scrape failed: ${error.message}`);
        throw error;
    }
    finally {
        await context.close();
...
async function postTweet(job) {
    const { content, mediaFiles } = job.data;
    logger_1.logger.info(`Posting tweet: ${content?.substring(0, 50)}...`);
    const context = await getContext();
    const page = await context.newPage();
    try {
        ...
        if (mediaFiles && mediaFiles.length > 0) {
            logger_1.logger.info(`Uploading ${mediaFiles.length} media files`);
        ...
        await page.waitForSelector('[data-testid="toast"]', { timeout: 8000 })
            .catch(() => logger_1.logger.info("Toast not found, assuming success"));
        logger_1.logger.info('Tweet posted successfully');
        return { success: true };
    }
    catch (error) {
        logger_1.logger.error(`Post tweet failed: ${error.message}`);
...
async function interact(job) {
    const { tweetId, action, content } = job.data; // action: 'like' | 'retweet' | 'reply'
    logger_1.logger.info(`Performing ${action} on tweet ${tweetId}`);
    const context = await getContext();
    const page = await context.newPage();
    try {
        ...
        if (action === 'like') {
            ...
            if (await likeButton.isVisible()) {
                ...
                logger_1.logger.info('Already liked or button not found');
            }
        }
        ...
        logger_1.logger.info(`Interaction ${action} complete`);
        return { success: true };
    }
    catch (error) {
        logger_1.logger.error(`Interaction failed: ${error.message}`);
...
async function follow(job) {
    ...
    logger_1.logger.info(`Following user ${username}`);
    const context = await getContext();
    const page = await context.newPage();
    try {
        ...
        if (await followButton.isVisible()) {
            ...
            logger_1.logger.info(`Followed ${username}`);
        }
        else if (await simplifiedButton.isVisible()) {
            await simplifiedButton.click();
            logger_1.logger.info(`Followed ${username} (simplified selector)`);
        }
        else {
            logger_1.logger.info(`Follow button not found, potentially already following`);
        }
        return { success: true };
    }
    catch (error) {
        logger_1.logger.error(`Follow failed: ${error.message}`);
        throw error;
    }
    finally {
        await context.close();
...
async function scrapeMentions(job) {
    logger_1.logger.info('Scraping mentions for auto-reply opportunities');
    const context = await getContext();
    const page = await context.newPage();
    const mentions = [];
    try {
        ...
        await page.evaluate(() => window.scrollBy(0, 1000));
        await page.waitForTimeout(5000);
        logger_1.logger.info(`Captured ${mentions.length} notification batches`);
        return {
            success: true,
        ...
        logger_1.logger.error(`Mentions scrape failed: ${error.message}`);
        throw error;
    }
    finally {
        await context.close();
