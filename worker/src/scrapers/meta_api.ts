...
export async function postFacebook(job: Job) {
...
export async function postInstagram(job: Job) {
...
export async function replyToComments(job: Job) {
...
export async function fetchAnalytics(job: Job) {
    const token = await getAccessToken();
    const pageId = await getPageId();
    const igUserId = await getIgUserId();
