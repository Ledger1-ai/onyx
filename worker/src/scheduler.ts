...
export class Scheduler {
    ...
    constructor(redis: IORedis, queue: Queue) {
        this.redis = redis;
        this.queue = queue;
    }
    ...
    public stop() {
        ...
        logger.info('Stopping Schedule Dispatcher...');
    }
    ...
    private async checkSchedule() {
        try {
            ...
            const schedule: DailySchedule = JSON.parse(scheduleJson);
            const now = new Date();
            ...
            for (const slot of schedule.slots) {
                ...
                if (slot.status === 'pending' && now >= startTime && now <= endTime) {
                    ...
                    logger.info(`Dispatching task: ${slot.activity_type} (Slot: ${slot.slot_id})`);
                    ...
                    if (jobName) {
                        ...
                        logger.warn(`No job mapping found for activity: ${slot.activity_type}`);
                    }
                }
            }
    ...
    private mapActivityToJob(activityType: string): string | null {
        switch (activityType) {
