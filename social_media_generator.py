...
Unified media generation functions for Twitter/X and LinkedIn posting.
Supersedes twitter_media_generator.py.
"""
...
# LinkedIn image size constants
LINKEDIN_SIZES = {
    "standard": (1200, 627),  # Link/Feed posts (approx 1.91:1)
    "square": (1080, 1080),   # Carousel/Mobile optimized (1:1)
    "portrait": (1080, 1350), # Mobile native (4:5)
}
...
def get_linkedin_image_size(size_type: str = "standard") -> Tuple[int, int]:
    """Get LinkedIn-optimized image dimensions"""
    return LINKEDIN_SIZES.get(size_type, LINKEDIN_SIZES["standard"])
...
def _resize_image_generic(image_path: str, target_width: int, target_height: int, suffix: str) -> Dict[str, Any]:
    """Generic image resizing logic"""
    try:
        with Image.open(image_path) as img:
            ...
            # Save resized image
            output_path = image_path.replace('.', f'_{suffix}.')
            img_cropped.save(output_path, 'JPEG', quality=95)
            
            logger.info(f"Image resized ({suffix}): {output_path}")
            return {
                "success": True,
...
def resize_for_twitter(image_path: str, target_size: str = "standard") -> Dict[str, Any]:
    ...
    width, height = get_twitter_image_size(target_size)
    return _resize_image_generic(image_path, width, height, f"twitter_{target_size}")

def resize_for_linkedin(image_path: str, target_size: str = "standard") -> Dict[str, Any]:
    """Resize an image for optimal LinkedIn posting"""
    width, height = get_linkedin_image_size(target_size)
    return _resize_image_generic(image_path, width, height, f"linkedin_{target_size}")
