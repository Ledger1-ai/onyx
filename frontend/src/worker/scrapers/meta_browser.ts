...
export async function watchStories(page: Page, platform: 'facebook' | 'instagram') {
    logger.info(`[Meta Browser] Watching ${platform} Stories...`);
    ...
    try {
        ...
        logger.info(`[Meta Browser] Successfully watched ${platform} stories.`);
        return { success: true, message: `Watched ${platform} stories`, screenshot: true };

    } catch (error: any) {
        logger.error(`[Meta Browser] Error watching stories: ${error.message}`);
        throw error;
    }
}

export async function watchReels(page: Page, platform: 'facebook' | 'instagram') {
    logger.info(`[Meta Browser] Watching ${platform} Reels...`);
    ...
    try {
        ...
        logger.info(`[Meta Browser] Successfully watched ${platform} reels.`);
        return { success: true, message: `Watched ${platform} reels`, screenshot: true };

    } catch (error: any) {
        logger.error(`[Meta Browser] Error watching reels: ${error.message}`);
        throw error;
    }
}
