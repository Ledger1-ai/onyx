...
async function getContext(userId?: string) {
    ...
    const profileName = userId ? `${userId}_twitter` : 'twitter_automation_profile';

    // Create isolated profile path
    // e.g. ../browser_profiles/65a123bcde/twitter_automation_profile
    // Or just ../browser_profiles/{userId}_twitter
...
export async function scrapeTwitter(job: IJobAdapter) {
    const { targetUrl, userId } = job.data;
...
export async function postTweet(job: IJobAdapter) {
    const { content, mediaFiles, userId } = job.data;
...
export async function interact(job: IJobAdapter) {
    const { tweetId, action, content, userId } = job.data; // action: 'like' | 'retweet' | 'reply'
...
export async function follow(job: IJobAdapter) {
...
export async function scrapeMentions(job: IJobAdapter) {
