 ...
 * Now creates a unique profile per user ID.
 */
async function getContext(userId?: string) {
    // If no userId provided, fall back to 'shared_default' (legacy behavior safety)
    const profileName = userId ? `${userId}_linkedin` : 'linkedin_automation_profile';
    const USER_DATA_DIR = path.resolve(process.cwd(), `../browser_profiles/${profileName}`);
    ...
    const browser = await playwrightExtra.launchPersistentContext(USER_DATA_DIR, {
...
// Helper: Check Login
async function checkLogin(page: any) {
    try {
...
/**
 * JOB: LinkedIn Post
 * Data: { text: string, mediaPath?: string, userId?: string }
 */
export async function postContent(job: IJobAdapter) {
    const { text, mediaPath, userId } = job.data;
    const context = await getContext(userId);
    const page = await context.newPage();

    try {
        if (!await checkLogin(page)) {
            logger.error(`[${job.id}] Not logged in to LinkedIn`);
            return { success: false, error: 'Not logged in' };
        }

        logger.info(`[${job.id}] Starting LinkedIn Post`);
        await page.goto('https://www.linkedin.com/feed/');
        await randomDelay();
        ...
        if (mediaPath) {
            try {
                logger.info(`[${job.id}] Uploading media: ${mediaPath}`);
                ...
                logger.warn(`[${job.id}] Media upload potentially failed: ${e}`);
            }
        }
        ...
        if (await postBtn.isEnabled()) {
            await postBtn.click();
            await randomDelay(3000, 5000);
            logger.info(`[${job.id}] LinkedIn Post Success`);
        ...
        logger.error(`[${job.id}] LinkedIn Post Failed: ${error.message}`);
        throw error;
    } finally {
        await context.close();
    }
...
/**
 * JOB: Engage Feed
 * Data: { count: number, userId?: string }
 */
export async function engageFeed(job: IJobAdapter) {
    const { count = 5, userId } = job.data;
    const context = await getContext(userId);
    const page = await context.newPage();

    try {
        if (!await checkLogin(page)) return { success: false, error: 'Not logged in' };

        logger.info(`[${job.id}] Engaging with ${count} posts on feed`);
        await page.goto('https://www.linkedin.com/feed/');
        await randomDelay();
        ...
        while (actions < count && scrolls < 10) {
            ...
            for (const btn of likeButtons) {
                ...
                if (await btn.isVisible() && Math.random() > 0.4) {
                    await btn.click();
                    actions++;
                    logger.info(`[${job.id}] Liked a post (${actions}/${count})`);
        ...
        logger.error(`[${job.id}] Feed Engagement Failed: ${error.message}`);
        throw error;
    } finally {
        await context.close();
    }
...
/**
 * JOB: Search & Engage
 * Data: { keyword: string, count: number, userId?: string }
 */
export async function searchAndEngage(job: IJobAdapter) {
    const { keyword, count = 3, userId } = job.data;
    const context = await getContext(userId);
    const page = await context.newPage();
    ...
    try {
        ...
        logger.info(`[${job.id}] Searching for "${keyword}"`);
        ...
        for (const btn of likeButtons) {
            ...
            if (await btn.isVisible()) {
                await btn.click();
                actions++;
                logger.info(`[${job.id}] Liked search result (${actions}/${count})`);
        ...
        logger.error(`[${job.id}] Search Engagement Failed: ${error.message}`);
        throw error;
    } finally {
        await context.close();
    }
 ...
 * Data: { maxCount: number, userId?: string }
 */
export async function scrapeNotifications(job: IJobAdapter) {
    const { maxCount = 5, userId } = job.data;
    const context = await getContext(userId);
    const page = await context.newPage();
    ...
    try {
        ...
        logger.info(`[${job.id}] Scraped ${notifications.length} notifications`);
        return { success: true, notifications };

    } catch (error: any) {
        logger.error(`[${job.id}] Notification Scrape Failed: ${error.message}`);
        throw error;
    } finally {
        await context.close();
    }
 ...
 * Data: { userId?: string }
 */
export async function scrapeAnalytics(job: IJobAdapter) {
    ...
    try {
        ...
        try {
            ...
            logger.warn('Could not find follower count via link, trying generic search');
        }

        logger.info(`[${job.id}] Scraped Analytics: ${count} followers`);
        return { success: true, followers: count };

    } catch (error: any) {
        logger.error(`[${job.id}] Analytics Scrape Failed: ${error.message}`);
        throw error;
    } finally {
        await context.close();
    }
